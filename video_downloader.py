...
class VideoDownloaderGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Downloader de Vídeos - TikTok, Instagram, YouTube")
        ...
        self.input_mode = tk.StringVar(value="file")  # "file" ou "paste"
    ...
    def setup_ui(self):
        ...
        self.paste_frame.pack_forget()  # Começa oculto
    ...
    def log(self, message, is_startup=False):
        ...
        # Print também no console para debug
        if is_startup:
            print(message)
    ...
    def start_download(self):
        ...
        # Executa em thread separada para não congelar interface
    ...
    def download_videos(self):
        ...
        try:
            ...
            # Lê links do arquivo ou do texto colado
            if self.input_mode.get() == "file":
            ...
            if not links:
                messagebox.showwarning("Aviso", "⚠️ Nenhum link encontrado no arquivo")
                self.log("✗ Arquivo de links está vazio")
                return
            ...
            for idx, link in enumerate(links, 1):
                ...
                try:
                    ...
                    # Comando yt-dlp com melhor qualidade
                    cmd = [
                        sys.executable,
                        "-m", "yt_dlp",
                        "-f", "bestvideo+bestaudio/best",  # Melhor qualidade
                        "-o", video_output,
                        "--no-warnings",
                        "--progress",
                        link
    ...
    def download_metadata(self, link, output_folder):
        """Baixa e salva metadados (título, descrição) do vídeo"""
        try:
            self.log(f"  → Baixando informações...")

            # Comando para extrair metadados em JSON
            cmd = [
                sys.executable,
                "-m", "yt_dlp",
                "--dump-json",
            ...
            if result.returncode == 0:
                ...
                # Se o arquivo já existe, adiciona número
                counter = 1
